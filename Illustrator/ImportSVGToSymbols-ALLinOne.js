// Import SVG Files as symbols and save it as library 


function getFolder() {
  return Folder.selectDialog('select Top folder:', Folder('~'));
}
...
function processTopFolder(topFolder) {
  ...
  ImportSVG2Symbols(topFolder, topFolder);  // process top folder first
  ...
  if (processRecursive) {
    for (var f = 0; f < listOfAssets.length; f++)
      if (listOfAssets[f] instanceof Folder) {
        ImportSVG2Symbols(listOfAssets[f], topFolder);
      }
      else continue;
  }
}
...
function prepareDocument() {
  ...
  var document = app.documents.add(DocumentColorSpace.RGB, width = 720 * mm, height = 720 * mm,);
...
function ImportSVG2Symbols(selectedFolder, topFolder) {

  if (selectedFolder) {

    var newLayer = rootDocument.layers.add();
    ...
    for (var i = 0; i < imageList.length; i++) {
      if (imageList[i] instanceof File) {
        ...
        if ((fileName.indexOf(".svg") == -1)) {
          ...
          var newSymnbol = rootDocument.symbols.add(newGroup);
